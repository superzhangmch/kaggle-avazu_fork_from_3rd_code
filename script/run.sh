...
pypy prep.py # train_c + fc + rare_d + id_map => train_pre: 重新生成样本数据train_pred。对于 device_id, device_ip, user_id 三个字段，如果是rare值，则生成计数特征；对于 user_id，如果只出现在一天，则统一记为v_id_s取值的特征
